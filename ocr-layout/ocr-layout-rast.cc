...
namespace ocropus {
    ...
    // FIXME put the graphics stuff into a separate source file --tmb
    ...
    // FIXME explain what this does --tmb
    ...
    void SegmentPageByRAST::segmentInternal(intarray &visualization,
        ...
        // FIXME
        // why does findGutters return a list of "colcandidates"?
        // are colcandidates gutters?  or what?
        // --tmb
        gutters->findGutters(colcandidates,whitespaceboxes,*charstats);
        gutters->filterOverlaps(columns,colcandidates);
        // it appears that this isn't a list of columns, but a list of
        // obstacles, so add the user-supplied obstacles as well
        for(int i=0;i<extra_obstacles.length();i++)
            columns.push(extra_obstacles[i]);
        if(debug_layout){
        ...
        // FIXME paragraph extraction should work generically on segmentation images.
        // It also doesn't belong inside the main RAST loop. --tmb
    ...
    void SegmentPageByRAST::segment(intarray &result, bytearray &in_not_inverted,rectarray &obstacles) {
    ...
    void SegmentPageByRAST::set(const char* var,double value) {
    ...
    void SegmentPageByRAST::depthFirstSearch(narray<bool> &lines_dag){
        //void visit (int k);
        int size = lines_dag.dim(0);
        val.resize(size);
        fill(val,false);
    ...
    // FIXME use descriptive name; "ro"??? --tmb
    ...
    // FIXME name getRelativeAlignment (?) --tmb
    ...
    // FIXME name getAlignment (?) or something else (?) --tmb
    ...
    // FIXME name matchesPrevious --tmb
    ...
    // FIXME name doesNotMatchPrevious --tmb
    ...
    // factor paragraph detection out of main RAST class
    // ideally, make paragraph detection work for arbitrary color segmentation
    // method too long
    // --tmb
    ...
    // FIXME
    // name getColumn (?) or something more descriptive
    // method too long
    // --tmb
    ...
    // FIXME
    // method too long
    // name: getColumn or more descriptive
    // functionality overlaps with other overloaded method?
    // --tmb
    ...
    // FIXME
    // refactor and move visualization methods out of the main RAST class
    // ideally, let visualization methods work on arbitrary inputs
    // --tmb
    ...
    void SegmentPageByRAST::color(intarray &image,bytearray &in,
        ...
        if(num_cols!=textcolumns.length() && debug_layout) {
