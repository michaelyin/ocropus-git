...
#define lua_readline(L,b,p)     ((void)L, ((b)=(char*)readline(p)) != NULL)
#define lua_saveline(L,idx) \
        if (lua_strlen(L,idx) > 0)  /* non-empty line? */ \
            add_history(lua_tostring(L, idx));  /* add it to history */
#define lua_freeline(L,b)       ((void)L, free(b))
#else
#define lua_readline(L,b,p)     ((void)L, fputs(p,stdout), fflush(stdout), b=(char*)malloc(10000),fgets(b,10000,stdin)!= NULL)
#define lua_saveline(L,idx) 0
#define lua_freeline(L,b)       ((void)L, free(b))
#endif
...
static int pmain(lua_State *L) {
    ...
    lua_setglobal(L, "libdir_temp_");
    // The following line converts the path into a Lua-acceptable one.
    luaL_loadstring(L, "package.path=(libdir_temp_..';')"
                        ":gsub(':',';'):gsub(';+',';')"
                        ":gsub(';','/?.lua;')"
                        // "..package.path"
    );
