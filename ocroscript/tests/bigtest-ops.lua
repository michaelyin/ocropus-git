...
function byte_op(image,rx,ry,op)
   local direct = bytearray()
   narray.copy(direct,image)
   if op=="erode" then iulib.binary_erode_rect(direct,rx,ry) end
   if op=="dilate" then iulib.binary_dilate_rect(direct,rx,ry) end
   if op=="close" then iulib.binary_close_rect(direct,rx,ry) end
   if op=="open" then iulib.binary_open_rect(direct,rx,ry) end
   return direct
end

function bits_op(image,rx,ry,op)
    local bi = bits.BitImage()
       ...
       for j,op in ipairs(ops) do
          ...
          for rx=1,17 do
             for ry = 1,17 do
                ...
                if eb then iulib.erase_boundary(truth,abs(rx),abs(ry),0) end
                dshow(truth,"a") 

                bi = bits_op(image,rx,ry,op)
                compare(truth,bi,rx,ry,file,op,"bits")
                if eb then iulib.erase_boundary(bi,abs(rx),abs(ry),0) end
                dshow(bi,"b")

                ri = rle_op(image,rx,ry,op)
                compare(truth,ri,rx,ry,file,op,"rle")
                if eb then iulib.erase_boundary(ri,abs(rx),abs(ry),0) end
                dshow(ri,"c")
